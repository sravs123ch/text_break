...
+const PAGE_MARGIN = 32; // Top and bottom margins
+const USABLE_PAGE_HEIGHT = PAGE_HEIGHT - (PAGE_MARGIN * 2);
+
+const shouldBreakElement = (element, currentPageHeight, remainingHeight) => {
+  if (!element) return false;
+  
+  const elementHeight = element.offsetHeight || element.getBoundingClientRect().height;
+  
+  // If element is taller than remaining space, it should break
...
+    const breaks = [];
+    let currentPageHeight = PAGE_MARGIN;
+    let currentPage = 1;
+    
+    children.forEach((child, index) => {
+      const elementHeight = getElementHeight(child);
+      const remainingHeight = USABLE_PAGE_HEIGHT - (currentPageHeight - PAGE_MARGIN);
+      
+      // Check if element should break to next page
+      if (shouldBreakElement(child, currentPageHeight, remainingHeight)) {
+        // For tables and images, move entire element to next page
+        if (isTableElement(child) || isImageElement(child)) {
+          breaks.push({
+            pageNumber: currentPage,
+            elementIndex: index,
+            breakBefore: true,
+            elementType: isTableElement(child) ? 'table' : 'image'
+          });
+          currentPage++;
+          currentPageHeight = PAGE_MARGIN + elementHeight;
+        } else {
+          // For text content, allow natural breaking
+          currentPage++;
+          currentPageHeight = PAGE_MARGIN + elementHeight;
+        }
+      } else {
...
+  // Enhanced CSS for proper page breaks
  useEffect(() => {
...
+      .editor-content .PlaygroundEditorTheme__table {
+        page-break-inside: avoid;
+        break-inside: avoid;
+        margin: 20px 0;
+      }
+      
+      .editor-content .PlaygroundEditorTheme__tableRow {
+        page-break-inside: avoid;
+        break-inside: avoid;
       }
     `;
...
+  // Apply page break classes based on calculated breaks
+  useEffect(() => {
+    if (!contentRef.current || pageBreaks.length === 0) return;
+    
+    const contentElement = contentRef.current;
+    const children = Array.from(contentElement.children);
+    
...
+      child.classList.remove('force-page-break');
+    });
+    
+    // Apply page break classes
+    pageBreaks.forEach(breakInfo => {
+      if (breakInfo.breakBefore && children[breakInfo.elementIndex]) {
+        children[breakInfo.elementIndex].classList.add('force-page-break');
+      }
+    });
+  }, [pageBreaks]);
